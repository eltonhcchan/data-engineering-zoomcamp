...
OPTIONS (
  format = 'PARQUET',
  uris = ['gs://kestra-de-zoomcamp-bucket/yellow_tripdata_2024-*.parquet']
);'''
...
- 65,623
- 840,402
- 20,332,093 (+, check table detail in BQ)
- 85,431,289
...
FROM de_zoomcamp.external_yellow_tripdata; (262)

SELECT count(DISTINCT(PULocationID))
FROM de_zoomcamp.external_yellow_tripdata_non_partitioned; (262)
...
SELECT PULocationID
FROM de_zoomcamp.external_yellow_tripdata_non_partitioned; This query will process 155.12MB

SELECT PULocationID, DOLocationID
FROM de_zoomcamp.external_yellow_tripdata_non_partitioned; This query will process 310.24MB
...
SELECT count(*)
FROM de_zoomcamp.external_yellow_tripdata_non_partitioned
WHERE fare_amount = 0;
...
0 B, it is answered entirely from the table's metadata.
